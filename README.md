...
![快速启动主界面：本地应用、文件夹、网页和命令混合展示](./docs/preview.png)

> 演示图包含本地应用、文件夹、网页和命令。实际使用时，本地文件与应用图标由 uTools 从系统读取，网页可获取站点图标；第三方品牌图标仅用于界面示例，不随插件打包。

## 主要功能

- 添加和启动本地文件、文件夹、网页与跨平台命令。
- macOS 使用登录式 zsh、Windows 使用 CMD、Linux 优先使用 Bash，并可为命令限定运行平台。
- 命令支持工作目录、环境变量、后台运行和终端运行，启动后会通知成功或失败状态，导入命令首次执行前需要确认。
